...
function statusBuild() {
    ...
    sendTele "$PATHJSON"
    sendInfo "Build Success. Total time elapsed: $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) seconds."
}
...
if [ "$upload_to_sf" = "release" ]; then
    ...
    sshpass -p '' scp "$FILEPATH" komodos@frs.sourceforge.net:/home/frs/project/komodos-rom/$DEVICE/
fi

if [ "$upload_to_sf" = "test" ]; then
    mv "$FILEPATH" $KOMODOFILE/komodo-beta/$DEVICE/
    sshpass -p '' scp "$FILEPATH" kry9ton@frs.sourceforge.net:/home/frs/project/krypton-project/Test/
fi
