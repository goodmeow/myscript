...
ROM_NAME="AncientOS"
ROM="ancient"
LINK_MANIFEST="https://github.com/ancient-lab/manifest"
BRANCH_MANIFEST="ten"
USER_SF="goodmeow"
...
git clone https://github.com/ancient-devices/device_xiaomi_sdm845-common -b ten device/xiaomi/sdm845-common
git clone https://github.com/ancient-devices/device_xiaomi_beryllium -b ancient/dev-ten device/xiaomi/beryllium
git clone https://github.com/GZR-Kernels/Optimus_Drunk_Beryllium kernel/xiaomi/beryllium
git clone https://github.com/ancient-devices/vendor_xiaomi_beryllium -b ten vendor/xiaomi
git clone https://gitlab.com/goodmeow/vendor_xiaomi_firmware -b ten vendor/xiaomi/firmware
...
function statusBuild() {
    if [ "$1" == "rom" ]; then
        cd $OUT
        FILEPATH=$(find -iname "${ROM_NAME}*.zip")
        if [[ ! -f $FILEPATH ]]; then
        ...
        sendTele "$FILEPATH"
        if [ "$2" == "Settings" ]; then sendTele $(find -iname "Komodo.apk"); fi
...
function sendSFtest() {
    ...
    sshpass -p '$(openssl enc -base64 -d <<< YWRtaW4xMjNic2QgCg==)' scp $FILEPATH ${USER_SF}@frs.sourceforge.net:/home/frs/project/goodmeow/test/
